...
public class ReportController extends BaseController{
	...
	public BackResult<List<Map<String,String>>> getTestHistoryReport(HttpServletRequest request,
			HttpServletResponse response, String userId,String month,String mobile,String token) {
		//返回结果
	...
	public BackResult<Boolean> deleteCvsByTime(HttpServletRequest request, HttpServletResponse response,String userId,String token,String mobile
			,String ids) {
		...
		try {
			...
			e.printStackTrace();
			logger.error("用户手机号：" + mobile + "执行批量删除检测记录异常，ids:" + ids + ",ip:" + super.getIpAddr(request) + ",异常信息为：" + e.getMessage());
			result.setResultCode(ResultCode.RESULT_FAILED);
			result.setResultMsg("系统异常");
		}
		
		logger.info("用户手机号：" + mobile + "执行批量删除检测记录成功，ids:" + ids + ",ip:" + super.getIpAddr(request));
		return result;
    }
	...
	@ResponseBody
	public BackResult<String> batchDownloadFile(HttpServletRequest request, HttpServletResponse response,String userId,String token,String mobile
			,String ids) {
		...
		try {
			...
			logger.error("用户【" + mobile + "】批量下载文件异常，ids:"  + ids +  ",ip: " + super.getIpAddr(request) + "，错误信息：" + e1.getMessage());
			e1.printStackTrace();
			result.setResultCode(ResultCode.RESULT_FAILED);
        ...
        try {
        	...
        	logger.error("用户【" + mobile + "】批量下载文件异常，ids:"  + ids +  ",ip: " + super.getIpAddr(request) + "，错误信息：" + e.getMessage());
            e.printStackTrace();
            result.setResultCode(ResultCode.RESULT_FAILED);
            result.setResultMsg("下载失败，服务器异常");
            return result;
        ...
        logger.info("用户【" + mobile + "】批量下载文件成功，ids:"  + ids +  ",ip: " + super.getIpAddr(request) +"，");
		return result;
	}
	
	@RequestMapping(value = "/getHistoryTestData")
	@ResponseBody
	public BackResult<Boolean> getHistoryTestData(HttpServletRequest request, HttpServletResponse response,String userId,String token,String mobile
