...
public class WsHttpUtils {
    private static final int REQUEST_TIMEOUT = 5 * 100; // 设置请求超时10秒钟

    private static final int CONNECT_TIMEOUT = 5 * 1000; // 连接超时时间

    private static final int SO_TIMEOUT = 2 * 1000; // 数据传输超时
    ...
    static {
        RequestConfig requestConfig = RequestConfig.custom()
                                                   ...
                                                   .build();
        client = HttpClients.custom().setDefaultRequestConfig(requestConfig).setMaxConnTotal(300).setMaxConnPerRoute(300).build();
    }
